...
export async function createOrder(formData: FormData) {
  ...
  const last = await prisma.order.findFirst({ where: { branchId }, orderBy: { number: "desc" } });
  const order = await prisma.order.create({
    data: {
      branchId, number: (last?.number ?? 0) + 1, type,
...
export async function addLine(formData: FormData) {
  await authorize("order.create");
  const { orderId, selection, qty, notes } = z
    .object({ orderId: z.string(), selection: z.string().min(1), qty: z.coerce.number().int().min(1), notes: z.string().optional() })
    .parse(Object.fromEntries(formData));
  await assertOrderMutable(orderId);
...
export async function removeOrderItem(formData: FormData) {
  ...
  const it = await prisma.orderItem.findUniqueOrThrow({ where: { id } });
...
export async function requestVoid(formData: FormData) {
  ...
  const { orderItemId, reason } = z
    ...
    .parse(Object.fromEntries(formData));
  const it = await prisma.orderItem.findUniqueOrThrow({ where: { id: orderItemId } });
  await assertOrderMutable(it.orderId);
